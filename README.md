...
3. Consumer эмулирует шлюз (2–5 с, ~90% успех), обновляет статус и **сам доставляет
   webhook** с ретраями (3 попытки, экспоненциальная задержка).
4. Если webhook не доставлен после всех попыток (или платёж не найден) — сообщение
   уходит в DLQ `payments.new.dlq`, откуда обработчик складывает его в таблицу
   `dead_letters` для последующего разбора и ручного реплея.
